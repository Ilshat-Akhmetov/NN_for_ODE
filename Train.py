...
def train_model(model, loss, optimizer, scheduler, num_epochs, train_domain,
                val_domain, func_approximation, differential_operator):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                relative_err_train.append(relative_error(true_analytical_solution_train,
                                                            func_preds).detach().numpy())
                abs_err_train.append(MaxAbsoluteError(true_analytical_solution_train,
                                                      func_preds).detach().numpy())
            #     writer.add_scalar("Loss train: ", epoch_loss, epoch)
                ...
                relative_err_valid.append(relative_error(true_analytical_solution_valid,
                                                            func_preds).detach().numpy())
                abs_err_valid.append(MaxAbsoluteError(true_analytical_solution_valid,
                                                      func_preds).detach().numpy())
